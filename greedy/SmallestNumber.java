package greedy;
// find the smallest number with given set of digit sum s and number of digits d
//Input  : s = 9, d = 2
//Output : 18
...
//	we apply greedy approach. We compare remaining sum with 9, if remaining sum is more than 9, 
//	we put 9 at the current position, else we put the remaining sum. Since we fill digits from right
//	to left, we put the highest digits on the right side. Below is implementation of the idea.
	
	// Function to print the smallest possible number with digit sum 's'
    // and 'd' number of digits
    static void findSmallest(int d, int s)
    {
        // If sum of digits is 0, then a number is possible
        // only if number of digits is 1
        if (s == 0)
        {
            System.out.print(d == 1 ? "Smallest number is 0" : "Not possible");
